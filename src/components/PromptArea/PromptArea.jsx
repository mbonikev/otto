...
function PromptArea({
  ...
  return (
    <div className="w-full h-fit flex flex-col gap-3 px-3 pb-3 z-0 sticky bottom-0 bg-white">
      <form
        onSubmit={submit}
        className="w-full max-w-[850px] mx-auto h-fit rounded-[25px] shadow-lg ring-1 p-2 ring-stone-200 overflow-hidden"
      >
        <textarea
          type="text"
          id="ProjectDesc"
          ...
          className="text-base font-norma bg-transparent min-h-[60px] max-h-[150px] px-3 pt-3 pb-4 outline-none w-full placeholder:text-dark-text-weak text-dark-text resize-none whitespace-pre-wrap "
        ></textarea>
        <div className="flex items-center justify-between w-full h-[36px] bg-red-400/0 min-h-[32px] px-1.5 mb-1.5">
          ...
          <div className="w-full h-full flex items-center justify-start gap-3">
            ...
            <div className="group cursor-pointer h-full w-fit px-3 flex items-center justify-center gap-1 text-sm ring-1 ring-stone-300 text-dark-text-weak hover:text-dark-text font-semibold rounded-full relative">
              ...
              Summarize
              <Tooltip title="Summarize text" placement="right-center" />
            </div>
          </div>
          {/* 2 */}

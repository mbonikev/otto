...
function Navbar({
  ...
  return (
    <div className="w-full h-[70px] min-h-[70px] bg-white dark:bg-body-dark grid grid-cols-3 text-dark-text dark:text-light-color px-4 sticky">
      {/* Overlay */}
      {chatsModal && (
        <div
          onClick={handleCloseChatsModal}
          className={`fixed top-0 left-0 w-full h-full bg-black/5 dark:bg-black/30 z-20 transition-opacity duration-300 ${
        ...
        {user ? (
          <>
            <button
              ...
              className="group h-12 w-auto aspect-square flex items-center justify-center text-2xl hover:bg-stone-100 text-dark-text-weak dark:text-light-color-weak hover:text-dark-text dark:hover:bg-card-dark-1 dark:hover:text-light-color rounded-full relative"
            >
              <HiOutlineChatBubbleLeftRight />
              <Tooltip
                title="Chat history"
      ...
      <div className="flex items-center justify-end gap-0 relative select-none">
        {user ? (
          <>
            <button className="group h-12 w-auto aspect-square flex items-center justify-center text-2xl hover:bg-stone-100 text-dark-text-weak hover:text-dark-text dark:text-light-color-weak dark:hover:bg-card-dark-1 dark:hover:text-light-color rounded-full relative">
              <BsIncognito />
            ...
            <button
              ...
              className="h-11 w-11 rounded-full overflow-hidden ml-3"
            >
              <img
                src={photo || avatar3}
                alt="avatar"
            ...
            {showPopup && (
              ...
              >
                <button className="w-full rounded-xl p-2 flex items-center justify-start gap-2 hover:bg-stone-100 text-dark-text-weak hover:text-dark-text dark:text-light-color-weak dark:hover:bg-light-color-weak/10 dark:hover:text-light-color">
                ...
                <button
                  ...
                  disabled={loading}
                  className="w-full rounded-xl p-2 flex items-center justify-start gap-2 hover:bg-stone-100 text-dark-text-weak hover:text-dark-text dark:text-light-color-weak dark:hover:bg-light-color-weak/10 dark:hover:text-light-color"
                >
                  {loading ? (
                    <>
                      <LuLoaderCircle className="text-2xl text-dark-text-weak/70 dark:text-light-color-weak/50 animate-spinLoader" />
                      <h1 className="text-sm">Sign Out</h1>
                    </>
                  ) : (
                    <>
                      <HiOutlineArrowRightStartOnRectangle className="text-2xl text-dark-text-weak/70 dark:text-light-color-weak/50" />
                      <h1 className="text-sm">Sign Out</h1>
                    </>
                  )}
                </button>
          ...
          <>
            ...
            <button
              onClick={() => (window.location = "/#/login")}
              className="group h-12 ml-2 w-fit px-6 flex items-center justify-center text-base font-medium bg-dark-text text-white rounded-full dark:text-dark-text dark:bg-white relative"
            >
              Login
            </button>
          </>
